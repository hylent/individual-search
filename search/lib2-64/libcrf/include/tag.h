...
namespace CRFPP {
	...
	protected:
		...
		const char *get_index(int in);
	        ...
	        const char* get_index(const char *&p, size_t pos); 

		
		//根据特征模板p和当前位置pos，拼合出特征函数feature_function
        //正常返回为0， 错误返回<0	
	    int CrfTag::apply_rule(char *feature_function, const char *p, size_t pos);
        ...
        //正常返回为0， 错误返回<0
		int buildFeatures();
		void build_feature_100();
		void  build_feature_200();
		int build_feature();
	...
	public:
		...
		{}
		*/
		explicit CrfTag(unsigned int this_max_nbest = DEFAULT_MAX_NBEST, unsigned int this_crf_max_word_num = DEFAULT_CRF_MAX_WORD_NUM, unsigned int this_crf_max_word_len = DEFAULT_CRF_MAX_WORD_LEN,unsigned int this_max_cols = DEFAULT_CRF_MAX_COLS, int this_template_type = 0) {
			crf_max_word_num = this_crf_max_word_num;
			crf_max_word_len = this_crf_max_word_len;
			crf_max_cols = this_max_cols;

			template_type = this_template_type;
			vlevel_ = 0;
			nbest_ = 0;
			cost_ = 0.0;
			Z_ = 0;
		...
		*/
		...
		//cols为列数,前row_len个串为第一列,第二个row_len串为第二列,以此类推
		//正常返回为0， 错误返回<0
		int add(char *row_p, unsigned int row_len,int cols = 1,int *wordner_token = NULL,
			       	int wordner_token_len = 0);

		//标注输入串过程，输出存于result[]中。如果设置了nbest>1，之后可以通过next获取nbest结果
		//正常返回为0， 错误返回<0		
		int parse(int use_wordner_opt = 0);
