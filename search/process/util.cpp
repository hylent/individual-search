...
*/
    ...
    std::string serialize_inverted_posting(InvertedPosting post){
        ...
        ret += num_to_string<size_t>(post.begin) + "#";
        ret += num_to_string<size_t>(post.end);
        return ret;
    }
    ...
    InvertedPosting unserialize_inverted_posting(std::string instr){
        ...
        post.begin = string_to_num<size_t>(vec[1]);
        post.end = string_to_num<size_t>(vec[2]);
        return post;
    }

    std::string serialize_doc_info(DocumentInfo info){
        std::vector<std::string> vec;
        vec.push_back(num_to_string<int>(info.doc_id));
       vec.push_back(num_to_string<int>(info.length));
       vec.push_back(num_to_string<int>(info.label));
       vec.push_back(info.title);
       vec.push_back(info.url);
