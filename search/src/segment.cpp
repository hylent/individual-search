...
int Segment::segment_string(char* str,std::vector<std::string>& token_vec){
    token_vec.clear();
    _wordseg->Cut(str, token_vec);
    #ifdef DEBUG
        //std::cout << "segment_result:" << limonp::Join(vec.begin(), vec.end(), "/") << std::endl;
        int count = token_vec.size();
        std::cout << " segment_result:";
