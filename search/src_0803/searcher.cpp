...
void Searcher::select_queryword(std::map<std::string, int>& token_tf, std::vector<QueryWord>& vec_query) {
    ...
    if (vec_query.size() > 0) {
        ...
        #ifdef DEBUG
            cout << "1st_must_word_id:" << vec_query[0].id << endl;
        #endif
    }
    if (vec_query.size() > 1) {
        vec_query[1].is_must = true;
        #ifdef DEBUG
            cout << "2nd_must_word_id:" << vec_query[1].id << endl;
        #endif
    }
}
...
void Searcher::parse_query(string query_str) {
    ...
    while (query_iter != vec_query.end()) {
        ...
        #ifdef DEBUG
            cout << "term_id:" << collector._word_id << " tf:" << collector._tf << " idf:" << collector._idf << endl;
        #endif
        query_iter++;
    }
    #ifdef DEBUG
