...
//对文档集合建立正排索引并输出
void Indexer::build_forward_index(char* raw_data_path, char* output_path){
    ...
    get_file_name(raw_data_path, DOC, output_name, FILE_NAME_SIZE);
    ...
    while(!feof(file)){
        ...
        //build_document_forward_index(doc_info_vec[3], doc_info_vec[0]);
        build_document_info(doc_info_vec, info);
        string info_str = serialize_doc_info(info);
        #ifdef DEBUG
            cout << "doc_info_str:" << info_str << endl;
        #endif
        fputs(info_str.c_str(), doc_file);
...
//对每篇文档建立brief信息
void Indexer::build_document_info(const std::vector<std::string>& doc_vec, DocumentInfo& info){
...
void Indexer::load_worddict(char* path) {
    ...
    while(!feof(file)){
        ...
        if (info.file_index > 0) {
            WordItem item(info.file_index, info.df, info.offset_begin, info.offset_end);
            _inverted_index.insert(std::pair<long, WordItem>(info.dict_id, item));
        }
    }   
    fclose(file);
}
...
void Indexer::build_inverted_index_from_file(char* data_path, char* dict_path, char* output_path){
    ...
    //加载已筛选过滤的词典
    get_file_name(dict_path, DICT, file_name, FILE_NAME_SIZE);
...
void Indexer::load_document_info(char* path) {
    ...
    while (!feof(file)) {
        ...
        explode(buffer_str, "\t", vec); 
        DocumentInfo info;
        info.doc_id = string_to_num<long>(vec[0]);
        info.subject = string_to_num<int>(vec[1]);
        info.spider_source = string_to_num<int>(vec[2]);
        info.doc_len = string_to_num<int>(vec[3]);
        _docs_info.insert(std::pair<long, DocumentInfo>(info.doc_id, info));
    }
...
void Indexer::load_term_index(size_t term_id) {
    WordItem* info = get_inverted_term(term_id);
    if (info == NULL || info->file_index == 0) {
        cout << "cannot find termindex " << term_id << endl;
        return;
    }
    ...
    char file_name[FILE_NAME_SIZE];
    get_file_name(_data_path, INDEX, file_name, FILE_NAME_SIZE, info->file_index);
    ...
    #ifdef TRACE
    long tmp_pos = 0;
    cout << "begin_load_term:" << term_id << " size:" << info->pos_begin << endl;
    ...
    #ifdef DEBUG
    cout << "end_load_term:" << term_id << " size:" << tmp_size << " total_size:" <<  info->pos_end <<endl;
    #endif
    cout << "end_load_term:" << term_id << " size:" << tmp_size << " total_size:" <<  info->pos_end <<endl;
}

void Indexer::load_inverted_index() {
    std::map<size_t, WordItem>::iterator iter = _inverted_index.begin();
    while (iter != _inverted_index.end()) {
        WordItem item = iter->second;
        if (item.file_index > 0) {
            load_term_index(iter->first);
            //load_term_index(2);
            //break;
        }
        iter++;
...
void Indexer::add_inverted_term(const size_t& term_id, const WordItem& item) {
    if (_inverted_index.find(term_id) == _inverted_index.end()) {
        _inverted_index.insert(std::pair<size_t, WordItem>(term_id, item));
    }
}

//成功取得下一个文档则返回true,否则为false
//获取ID>= doc_id的文档信息
bool Indexer::get_next_doc(size_t word_id, long doc_id, DocumentItem& item) {
    WordItem* word_item = get_inverted_term(word_id);
    if (word_item == NULL) {
        return false;
    }
    if (word_item->load_type == NOT_LOAD) {
        load_term_index(word_id);
    }
    if (word_item->pos_cur < 0) {
        word_item->pos_cur = word_item->pos_begin;
    }
    while (word_item->pos_cur < word_item->pos_end) {
        const InvertedPosting& post_item = _inverted_posting_list[word_item->pos_cur];
    ...
    if (word_item->pos_cur == word_item->pos_end) {
        return false;
    }
    return true;
}
